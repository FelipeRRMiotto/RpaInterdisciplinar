...
for amb in range(0,2):
    ...
    try:
        ...
        cur1.execute('select dt_data_inicio, dt_data_final, var_nome, var_local, num_preco_ticket, pk_int_id_evento, fk_int_id_usuario, deletedat from tb_evento where createdAt = %s',(ontem,))
        ...
        cur1.execute('select pk_int_id_cor_mascote, text_fundo, text_secundaria, text_primaria, deletedAt from tb_cor_mascote where updatedat = %s order by pk_int_id_cor_mascote',(ontem,))
        cores = cur1.fetchall()
        ...
        if len(cores) > 0:
            ...
            for i in cores2:
                for c in range(0,len(i)):
                    if i[c] != cores[c+1]:
                        lista_mods.append([lista_cols[c],cores[c+1],i[0]])
        ...
        cur1.execute('select pk_int_id_evento, dt_inicio, dt_final, var_nome, var_local, num_preco_ticket, deletedat, fk_int_id_usuario from tb_evento where updatedat = %s order by pk_int_id_evento',(ontem,))
        eventos = cur1.fetchall()

        #Verificando se algum novo registro foi encontrado
        if len(eventos) > 0:
            lista_cols = ["dt_inicio", "dt_final", "var_nome", "var_local", "float_preco_ticket", "deletedat", "fk_int_id_usuario"]
            lista_mods = []
            querySelect = "select dt_inicio, dt_final, var_nome, var_local, num_preco_ticket, deletedat, fk_int_id_usuario from tb_evento where pk_int_id_evento in ("
            for i in eventos:
                querySelect = querySelect+i[0]+","
            ...
            for i in eventos2:
                for c in range(0,len(i)):
                    if i[c] != eventos[c+1]:
                        if c in [4,6]:
                            lista_mods.append([lista_cols[c],eventos[c+1],i[0]])
                        else:
                            lista_mods.append([lista_cols[c],"'"+str(eventos[c+1])+"'",i[0]])
        ...
        cur1.execute('select pk_int_id_barraca, var_nome, deletedat, fk_int_id_evento from tb_barraca where updatedat = %s order by pk_int_id_barraca',(ontem,))
        barracas = cur1.fetchall()
        ...
        if len(barracas) > 0:
            ...
            for i in barracas2:
                for c in range(0,len(i)):
                    if i[c] != barracas[c+1]:
                        if c == 2:
                            lista_mods.append([lista_cols[c],barracas[c+1],i[0]])
                        else:
                            lista_mods.append([lista_cols[c],"'"+str(barracas[c+1])+"'",i[0]])
            
            for i in lista_mods:
                query = ("update tb_barracas set "+str(i[0])+" = "+str(i[1])+" where pk_int_id_barraca = "+str(i[2])).replace("None","null")
                cur2.execute(query)
            conn2.commit()
